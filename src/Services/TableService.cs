...
public class TableService(IConfiguration configuration)
{
	...
	public async Task<Token?> GetTokenDocumentAsync(string tokenId)
	{
		...
		var entities =
			tableClient.QueryAsync<DocumentEntity>(ent =>
				ent.PartitionKey == AuthTokenPartitionKey && ent.RowKey == tokenId);
		await foreach (var entity in entities) return SerializationService.Deserialize<Token>(entity.Document);
	...
	public async Task<User?> GetUserDocumentByUsernameAsync(string username)
	{
		...
		var entities =
			tableClient.QueryAsync<DocumentEntity>(ent =>
				ent.PartitionKey == UsersPartitionKey && ent.RowKey == username);

		await foreach (var entity in entities) return SerializationService.Deserialize<User>(entity.Document);
	...
	public async Task<User?> GetUserDocumentByUserIdAsync(string userId)
	{
		...
		var entities =
			tableClient.QueryAsync<DocumentEntity>(ent =>
				ent.PartitionKey == UsersPartitionKey && ent.UserId == userId);
	...
	public async Task<Account?> GetAccountDocumentAsync(string accountId)
	{
		...
		var entities = tableClient.QueryAsync<DocumentEntity>(ent =>
			ent.PartitionKey == AccountsPartitionKey && ent.RowKey == accountId);
		await foreach (var entity in entities) return SerializationService.Deserialize<Account>(entity.Document);
	...
	public async Task<IList<Account>> GetAccountDocumentsAsync(string userId)
	{
		...
		var entities =
			tableClient.QueryAsync<DocumentEntity>(ent =>
				ent.PartitionKey == AccountsPartitionKey && ent.UserId == userId);
